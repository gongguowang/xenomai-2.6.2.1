...
#else
	nsamples = ONE_BILLION / (unsigned int)period_ns;
#endif
...
#else
	start_ticks = timer_info.date + rt_timer_ns2ticks(3000000);
	expected_tsc = timer_info.tsc + rt_timer_ns2tsc(3000000);
#endif
...
#else
		err = rt_sem_create(&display_sem, sem_name, 1, S_FIFO);
#endif
 		if (err) {
			fprintf(stderr,
